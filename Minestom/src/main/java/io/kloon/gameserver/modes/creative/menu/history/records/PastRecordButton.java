...
public class PastRecordButton extends AsyncPlayerButton<PastRecordButton.Data> {
    private final ChangeRecord record;

    public PastRecordButton(ChestMenu menuOfButton, int slot, ChangeRecord record) {
    ...
    public CompletableFuture<Data> fetchData(Player player) {
        return Kgs.getCaches().monikers().getByMinecraftUuid(record.author()).thenApply(Data::new);
    }
